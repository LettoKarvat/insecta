...
export function FAESPdfDoc({ printable }: { printable: FaesPrintable }) {
  ...
  const LOGO_SRC = isValidImage ? RAW_LOGO : "/logo.jpg"; // deixe "/logo.jpg" em /public como fallback
  ...
  // ─── MAPEAMENTO DE DEMANDA (novos campos + fallback ao que já existe) ─────
  ...
  const monitoramentoGarantia: string =
    ...
    )} ano(s). Reinspeção periódica conforme contrato. Laudo técnico e certificado serão emitidos.`;
  ...
  const Page1 = (
    <Page size="A4" style={S.page}>
      ...
      <View style={S.header}>
        ...
        <View style={S.headerRight}>
          ...
          <View style={S.chipRow}>
            ...
            <Text style={S.chip}>Emissão: {fmtDateBR(createdAt)}</Text>
      ...
      {/* Escopo / Mapeamento de Demanda (chamada) */}
  ...
  /* ───────────────── PÁGINA 3 – MAPEAMENTO DE DEMANDA ───────────────── */
  const Page3 = (
    <Page size="A4" style={S.page}>
  ...
  const validadeStr = addYearsBR(
    d.data_emissao || printable.faes?.created_at || "",
    validadeAnos
  );
  const Page4 = (
    <Page size="A4" style={S.page}>
      {/* Header compacto para o certificado */}
      ...
      <View style={S.box}>
        <Text style={S.text}>
          ...
          do certificado: {validadeAnos} ano(s)
          {d.data_emissao ? ` (até ${validadeStr})` : ""}.
        </Text>
        <Text style={[S.text, { marginTop: 6 }]}>
          Licença Sanitária:{" "}
